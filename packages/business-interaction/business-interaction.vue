		...
		<view class="page-header">
			<view class="header-content">
				<view class="header-title">
					<text class="title-text">商机互动</text>
					<text class="title-subtitle">把握每一次商业机会</text>
				</view>
		...
		<view class="list-container">
			...
			<view class="comment-card" v-for="item in commentList" :key="item.id">
				...
				<view class="card-header">
					...
					<!-- 是否本人标识 -->
					<view class="owner-badge" v-if="item.owner === 1">
				...
				<view class="target-section" @click="goToTarget(item)">
					...
					<view class="target-content">
						<view class="target-label">评论了你的商机</view>
				...
				<view class="card-actions">
					<view class="reply-btn" @click="handleReply(item)">
						<uni-icons type="chatbubble" size="16" color="#FF8700"></uni-icons>
						<text class="reply-text">回复评论</text>
			...
			<view v-if="commentList.length === 0 && loadingStatus === 'noMore'" class="empty-state">
				...
				<text class="empty-text">暂无商机评论</text>
				<text class="empty-tip">发布商机后，商友们会在这里留下评论</text>
			</view>
		</view>
	</view>
</template>
...
<script setup>
	...
	const CommentApi = {
		/** 获取评论分页列表 */
		getCommentPage: (params) => request('/app-api/member/comment/my-page-by-target-type', {
			method: 'GET',
			data: params
		})
	};
...
<style lang="scss" scoped>
	...
	.card-header {
		...
		.owner-badge {
			...
			align-items: center;
			gap: 4rpx;
			background: linear-gradient(135deg, $theme-color 0%, #FF6B00 100%);
			color: #fff;
			font-size: 20rpx;
			padding: 6rpx 16rpx;
			border-radius: 20rpx;
			font-weight: 500;
			box-shadow: 0 2rpx 8rpx rgba(255, 135, 0, 0.25);
		}
	}
	...
	.card-actions {
		...
		.reply-btn {
			...
			gap: 8rpx;
			background: linear-gradient(135deg, #FFF9F5 0%, #FFF5E8 100%);
			border: 1rpx solid rgba(255, 135, 0, 0.2);
			border-radius: 32rpx;
			padding: 10rpx 24rpx;
			transition: all 0.2s;
