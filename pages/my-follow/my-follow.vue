	...
	<view class="followed-users-page">
		...
		<scroll-view scroll-y class="content-scroll" refresher-enabled :refresher-triggered="refreshing"
			...
			<view v-if="followedUsers.length > 0" class="user-list">
				...
				-->
				<view v-for="item in followedUsers" :key="item.id" class="user-card"
...
<script setup>
	...
	const getFollowedUsers = async (isRefresh = false) => {
		...
		try {
			const {
				data,
				error
			} = await request('/app-api/member/follow/page', {
				method: 'GET',
				data: params
			});
	...
	/**
	 * 查看用户详情（名片页）
	 * @param {object} user - 包含用户ID、昵称、头像等信息的对象
	 */
	const viewUserDetail = (user) => {
		...
		console.log('跳转到名片页（标准流程），URL:', url);
