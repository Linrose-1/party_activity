<template>
	<view class="refund-apply-page">
		...
		<view class="info-banner">
			...
			<text>{{ staticWords.tipText }}</text>
		</view>

		<!-- 聚会信息卡片 -->
		<view class="card activity-card">
			<view class="card-title">{{ staticWords.sectionTitle1 }}</view>
			...
			<view class="refund-amount">
				<text class="label">{{ staticWords.refundAmountLabel }}</text>
		...
		<view class="card upload-card">
			<view class="card-title">{{ staticWords.sectionTitle2 }}</view>
			<view class="upload-notice">
				<text>{{ staticWords.uploadDesc }}</text>
			</view>
			<view class="file-picker-wrapper">
				<uni-file-picker v-model="qrCodeImage" file-mediatype="image" mode="grid" :limit="1"
					@select="handleImageSelect" @delete="handleImageDelete" :title="staticWords.uploadBtnText" />
			</view>
		</view>

		<!-- 底部提交栏 -->
		<view class="bottom-bar">
			<button class="submit-btn" @click="handleSubmit" :disabled="!qrCodeUrl">{{ staticWords.submitBtnText }}</button>
		</view>
	</view>
</template>
