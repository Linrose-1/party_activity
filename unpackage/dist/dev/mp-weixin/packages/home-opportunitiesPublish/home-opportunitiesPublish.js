...
if (!Math) {
  (_easycom_uni_data_select + _easycom_uni_icons + _easycom_DragImageUploader + SmartGuidePopup)();
}
  ...
  setup(__props) {
    ...
    const saveDraft = (data) => {
      if (data.title || data.content || data.tags.length > 0 || data.images.length > 0) {
        ...
        common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:449", "📝 草稿已自动保存");
      }
    };
    const checkDraft = () => {
    ...
    const clearDraft = () => {
      common_vendor.index.removeStorageSync(DRAFT_KEY);
      common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:478", "🧹 草稿已清除");
    };
        ...
        if (data.postType == 1) {
          ...
          if (data.expectedInvestment) {
            try {
              ...
              common_vendor.index.__f__("error", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:536", "解析 expectedInvestment 失败:", e);
            }
          }
        }
        if (data.postVideo) {
    ...
    const checkPublishQuota = async () => {
      try {
        ...
        common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:576", `权益加载完成: 商机=${quotaBusiness.value}, 猎伙=${quotaPartner.value}`);
      } catch (e) {
        common_vendor.index.__f__("error", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:578", "获取权益失败", e);
      }
    };
...
请充值后再发布，让优质机会不被错过。`,
    ...
    async function logTagSearch(name, type) {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:789", `标签历史 "${tagName}" 已记录`);
      } catch (error) {
        common_vendor.index.__f__("error", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:791", "记录标签历史失败:", error);
      }
    }
    ...
    async function fetchTagSuggestions(keyword) {
      try {
        ...
        common_vendor.index.__f__("error", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:832", "获取标签建议失败:", e);
        tagSuggestions.value = [];
      }
    }
        ...
        success: async (res) => {
          ...
          results.forEach((result) => {
            if (result.data)
              ...
              common_vendor.index.__f__("error", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:874", "上传失败:", result.error);
          });
    ...
    async function handleChooseVideo() {
      ...
      common_vendor.index.chooseVideo({
        ...
        fail: (err) => {
          common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:985", "取消选择视频");
        }
      });
    }
      ...
      common_vendor.index.chooseMedia({
        ...
        success: (res) => {
          ...
          common_vendor.wx$1.cropImage({
            ...
            cropScale: "4:3",
            success: (cropRes) => {
              common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:1019", "裁剪成功:", cropRes.tempFilePath);
              uploadCoverToCloud(cropRes.tempFilePath);
            },
            fail: (err) => {
              common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:1023", "用户取消裁剪或失败:", err);
            }
          });
        }
      });
    ...
    const fetchMyEnterprises = async () => {
      ...
      if (!error && data && data.list) {
        ...
        common_vendor.index.__f__("log", "at packages/home-opportunitiesPublish/home-opportunitiesPublish.vue:1123", "✅ 可用发布身份企业数:", myEnterprises.value.length);
      }
    };
    ...
    function submitPost() {
      ...
      if (form.topic === "创业猎伙") {
        ...
        if (form.partnerTypes.includes("4") && !form.partnerTypeOther.trim())
          return common_vendor.index.showToast({
      ...
      const postData = {
        ...
        ...form.topic === "创业猎伙" || form.topic === "商机分享+创业猎伙" ? {
          ...
          partnerTypes: form.partnerTypes.map((v) => v === "4" && form.partnerTypeOther.trim() ? form.partnerTypeOther.trim() : v).join(","),
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        y: form.topic === "商机分享+创业猎伙",
        z: isEditMode.value,
        A: isEditMode.value ? "#999" : "#333",
        B: common_vendor.o(topicChange),
        C: isEditMode.value
      }, isEditMode.value ? {} : {}, {
        D: form.topic === "创业猎伙" || form.topic === "商机分享+创业猎伙"
      }, form.topic === "创业猎伙" || form.topic === "商机分享+创业猎伙" ? common_vendor.e({
        E: common_vendor.f(partnerTypeOptions, (item, k0, i0) => {
          return common_vendor.e({
            ...
            e: item.value,
            f: common_vendor.o(($event) => togglePartnerType(item.value), item.value)
          });
        }),
        F: form.partnerTypes.includes("4")
      }, form.partnerTypes.includes("4") ? {
        G: form.partnerTypeOther,
        H: common_vendor.o(($event) => form.partnerTypeOther = $event.detail.value)
      } : {}, {
        I: common_vendor.f(urgentLevelOptions, (item, k0, i0) => {
          return {
        ...
        J: form.investmentFund,
        K: common_vendor.o(($event) => form.investmentFund = $event.detail.value),
        L: form.investmentResource,
        M: common_vendor.o(($event) => form.investmentResource = $event.detail.value),
        N: form.investmentEquity,
        O: common_vendor.o(($event) => form.investmentEquity = $event.detail.value),
        P: !isEditMode.value && usePointForPublish.value
      }, !isEditMode.value && usePointForPublish.value ? {} : {}) : {}, {
        Q: common_vendor.f(form.tags, (tag, index, i0) => {
          return {
        ...
        R: tagSuggestions.value.length > 0
      }, tagSuggestions.value.length > 0 ? {
        S: common_vendor.f(tagSuggestions.value, (suggestion, index, i0) => {
          return {
        ...
        T: form.tagInput,
        U: common_vendor.o(($event) => form.tagInput = $event.detail.value),
        V: common_vendor.o(handleAddTagManually),
        W: form.images.length === 0 && !form.postVideo
        ...
        aa: common_vendor.o(handleChooseVideo)
      } : {}, {
        ab: form.mediaType === "image" && form.images.length > 0
      }, form.mediaType === "image" && form.images.length > 0 ? {
        ac: common_vendor.o(handleChooseImage),
        ad: common_vendor.o(($event) => form.images = $event),
        ae: common_vendor.p({
          ["max-count"]: 9,
          modelValue: form.images
        })
      } : {}, {
        af: form.mediaType === "video" && form.postVideo
      }, form.mediaType === "video" && form.postVideo ? common_vendor.e({
        ag: form.postVideo,
        ah: common_vendor.o(deleteVideo),
        ai: form.businessCoverImageUrl
      }, form.businessCoverImageUrl ? {
        aj: form.businessCoverImageUrl
      } : {
        ak: common_vendor.p({
          type: "image",
          size: "24",
        ...
        al: common_vendor.o(handleChooseVideoCover)
      }) : {}, {
        am: common_vendor.t(form.mediaType === "image" ? "最多可上传9张图片" : "仅支持上传一个视频"),
        an: form.showProfile,
        ao: common_vendor.o((e) => form.showProfile = e.detail.value),
        ap: form.isReadTrace === 1,
        aq: common_vendor.o((e) => form.isReadTrace = e.detail.value ? 1 : 0),
        ar: common_vendor.t(isEditMode.value ? "提交修改" : "发布帖子"),
        as: isQuotaLoaded.value && currentRemainingQuota.value <= 0 && !isEditMode.value ? 1 : "",
        at: common_vendor.o(handleSubmitClick),
        av: common_vendor.sr(smartGuidePopupRef, "4f014bb0-5", {
